...
Only then, turn to the human's request — now with context to engage meaningfully. When reporting that orientation is complete, follow `notes/conversational-orientation-handback.md`: state readiness in short natural prose, explain any degraded boundary plainly, and mention the prior lane or ask a steering question only when relevant. Do not use an aligned status strip by default. Keep startup receipts out unless something failed or Or asks for them.
