#!/usr/bin/env bash
# Status fact collection and rendering for github:ci:* tasks.

github_ci_print_row() {
  printf '%s\t%s\t%s\t%s\t%s\n' "$1" "$2" "$3" "$4" "$5"
}

github_ci_write_secret_status() {
  ...
  github_ci_print_row Repo Agent Secret Status Detail > "$out"
  while IFS=$'\t' read -r repo agent; do
    ...
    if ! github_ci_secret_names_file "$repo" "$agent" "$names_file" 2>"$err_file"; then
      ...
      error_secret=$(github_ci_error_secret_name "$scope")
      github_ci_print_row "$repo" "$agent" "$error_secret" "error" "$detail" >> "$out"
      missing=1
      continue
    fi

    github_ci_required_secret_names "$scope" "$agent" > "$required_file"
    while IFS= read -r secret; do
      [ -n "$secret" ] || continue
      if github_ci_secret_present "$names_file" "$secret"; then
        github_ci_print_row "$repo" "$agent" "$secret" "present" "" >> "$out"
      else
        github_ci_print_row "$repo" "$agent" "$secret" "missing" "" >> "$out"
        missing=1
      fi
    done < "$required_file"
  done < "$targets_file"
...
github_ci_print_secret_status() {
  local targets_file="$1" scope="$2" tmp_dir="$3"
  local rows_file="$tmp_dir/secret-status.tsv" status=0

  github_ci_write_secret_status "$targets_file" "$scope" "$rows_file" "$tmp_dir" || status=1
  cat "$rows_file"
  return "$status"
}

github_ci_pi_auth_source_status() {
  local source="$1"
  ...
  if ! "$GITHUB_CI_JQ_BIN" -e 'type == "object" and length > 0' "$source" >/dev/null 2>&1; then
...
github_ci_print_pi_auth_source_status() {
  local source="$1" result source_status source_detail status=0

  if ! result=$(github_ci_pi_auth_source_status "$source"); then
    status=1
  fi
